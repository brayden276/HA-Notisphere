...
from .recipients.coordinator import HomeAssistantRecipientDiscoveryCoordinator
...
async def async_setup_entry(hass: Any, entry: Any) -> bool:
    ...
    recipient_discovery = HomeAssistantRecipientDiscoveryCoordinator(
        hass,
        manager,
        HomeAssistantRecipientDiscovery(hass),
    )
    domain_data = hass.data.setdefault(DOMAIN, {})
    try:
        # Subscribe first so a late Mobile App service registration cannot fall
        # between the initial snapshot and event-driven discovery.
        await recipient_discovery.async_start()
        await recipient_discovery.async_refresh(reconcile_health=False)
        ...
        domain_data[entry.entry_id] = {
            ...
            DATA_RECIPIENT_DISCOVERY: recipient_discovery,
        }
        async_register_websocket_commands(hass)
        await async_register_panel(hass)
    except Exception:
        domain_data.pop(entry.entry_id, None)
        await recipient_discovery.async_stop()
...
async def async_unload_entry(hass: Any, entry: Any) -> bool:
    ...
    if entry_data is not None:
        await entry_data[DATA_RECIPIENT_DISCOVERY].async_stop()
